@extends('layouts._default')
@section('title-page', '列表')

@section('sub_header_title1', '基础Base')

@section('link-css')

@stop

@section('content')

    <div class="row">
        <div class="col-lg-6">
            <!--begin::Portlet-->
            <div class="m-portlet">
                <div class="m-portlet__head">
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                                Timeline List
                            </h3>
                        </div>
                    </div>
                </div>
                <div class="m-portlet__body">
                    <!--begin::Section-->
                    <div class="m-section">
                        <h3 class="m-section__heading">
                            Default Timelime
                        </h3>
                        <div class="m-section__content">
                            <!--begin::Preview-->
                            <div class="m-demo" data-code-html="true" data-code-js="false" data-code-preview="true">
                                <div class="m-demo__preview">
                                    <div class="m-list-timeline">
                                        <div class="m-list-timeline__items">
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    12 new users registered and pending for activation
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    Just now
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Scheduled system reboot completed
                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                        completed
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    14 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    New order has been planced and pending for processing
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    20 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Database server overloaded 80% and requires quick reboot
                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                        settled
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    1 hr
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    System error occured and hard drive has been shutdown -
                                                    <a class="m-link" href="#">
                                                        Check
                                                    </a>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    2 hrs
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Production server is rebooting...
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    3 hrs
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Preview-->
                            <!--begin::Dropdown-->
                            <div class="m-dropdown m-dropdown--inline m-dropdown--huge m-dropdown--arrow" data-dropdown-toggle="click">
                                <a class="m-dropdown__toggle btn btn-success dropdown-toggle" href="#">
                                    Dropdown example
                                </a>
                                <div class="m-dropdown__wrapper">
                                    <span class="m-dropdown__arrow m-dropdown__arrow--left">
                                    </span>
                                    <div class="m-dropdown__inner">
                                        <div class="m-dropdown__body">
                                            <div class="m-dropdown__content">
                                                <div class="m-scrollable" data-max-height="200" data-scrollable="true">
                                                    <div class="m-list-timeline">
                                                        <div class="m-list-timeline__items">
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    12 new users registered and pending for activation
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    Just now
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Scheduled system reboot completed
                                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                                        completed
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    14 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    New order has been planced and pending for processing
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    20 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Database server overloaded 80% and requires quick reboot
                                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                                        settled
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    1 hr
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    System error occured and hard drive has been shutdown -
                                                                    <a class="m-link" href="#">
                                                                        Check
                                                                    </a>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    2 hrs
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Production server is rebooting...
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    3 hrs
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Dropdown-->
                        </div>
                    </div>
                    <!--end::Section-->
                    <!--begin::Section-->
                    <div class="m-section">
                        <h3 class="m-section__heading">
                            State Colors
                        </h3>
                        <div class="m-section__content">
                            <!--begin::Preview-->
                            <div class="m-demo" data-code-html="true" data-code-js="false" data-code-preview="true">
                                <div class="m-demo__preview">
                                    <div class="m-list-timeline">
                                        <div class="m-list-timeline__items">
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    12 new users registered and pending for activation
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    Just now
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--danger">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Scheduled system reboot completed
                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                        completed
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    14 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--warning">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    New order has been planced and pending for processing
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    20 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--primary">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Database server overloaded 80% and requires quick reboot
                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                        settled
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    1 hr
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--brand">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    System error occured and hard drive has been shutdown -
                                                    <a class="m-link" href="#">
                                                        Check
                                                    </a>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    2 hrs
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Production server is rebooting...
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    3 hrs
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Preview-->
                            <!--begin::Dropdown-->
                            <div class="m-dropdown m-dropdown--inline m-dropdown--huge m-dropdown--arrow" data-dropdown-toggle="click">
                                <a class="m-dropdown__toggle btn btn-danger dropdown-toggle" href="#">
                                    Dropdown example
                                </a>
                                <div class="m-dropdown__wrapper">
                                    <span class="m-dropdown__arrow m-dropdown__arrow--left">
                                    </span>
                                    <div class="m-dropdown__inner">
                                        <div class="m-dropdown__body">
                                            <div class="m-dropdown__content">
                                                <div class="m-scrollable" data-max-height="200" data-scrollable="true">
                                                    <div class="m-list-timeline">
                                                        <div class="m-list-timeline__items">
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    12 new users registered and pending for activation
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    Just now
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Scheduled system reboot completed
                                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                                        completed
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    14 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    New order has been planced and pending for processing
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    20 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Database server overloaded 80% and requires quick reboot
                                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                                        settled
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    1 hr
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    System error occured and hard drive has been shutdown -
                                                                    <a class="m-link" href="#">
                                                                        Check
                                                                    </a>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    2 hrs
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Production server is rebooting...
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    3 hrs
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Dropdown-->
                        </div>
                    </div>
                    <!--end::Section-->
                    <!--begin::Section-->
                    <div class="m-section">
                        <h3 class="m-section__heading">
                            Item Icons
                        </h3>
                        <div class="m-section__content">
                            <!--begin::Preview-->
                            <div class="m-demo" data-code-html="true" data-code-js="false" data-code-preview="true">
                                <div class="m-demo__preview">
                                    <div class="m-list-timeline">
                                        <div class="m-list-timeline__items">
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-user">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    12 new users registered and pending for activation
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    Just now
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--danger">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-interface-7">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Scheduled system reboot completed
                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                        completed
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    14 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--warning">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-placeholder">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    New order has been planced and pending for processing
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    20 mins
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--primary">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-share">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Database server overloaded 80% and requires quick reboot
                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                        settled
                                                    </span>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    1 hr
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--brand">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-exclamation-1">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    System error occured and hard drive has been shutdown -
                                                    <a class="m-link" href="#">
                                                        Check
                                                    </a>
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    2 hrs
                                                </span>
                                            </div>
                                            <div class="m-list-timeline__item">
                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                </span>
                                                <span class="m-list-timeline__icon flaticon-cogwheel-2">
                                                </span>
                                                <span class="m-list-timeline__text">
                                                    Production server is rebooting...
                                                </span>
                                                <span class="m-list-timeline__time">
                                                    3 hrs
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Preview-->
                            <!--begin::Dropdown-->
                            <div class="m-dropdown m-dropdown--up m-dropdown--inline m-dropdown--huge m-dropdown--arrow" data-dropdown-toggle="click">
                                <a class="m-dropdown__toggle btn btn-brand dropdown-toggle" href="#">
                                    Dropdown example
                                </a>
                                <div class="m-dropdown__wrapper">
                                    <div class="m-dropdown__inner">
                                        <div class="m-dropdown__body">
                                            <div class="m-dropdown__content">
                                                <div class="m-scrollable" data-max-height="200" data-scrollable="true">
                                                    <div class="m-list-timeline">
                                                        <div class="m-list-timeline__items">
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-user">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    12 new users registered and pending for activation
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    Just now
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--danger">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-interface-7">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Scheduled system reboot completed
                                                                    <span class="m-badge m-badge--success m-badge--wide">
                                                                        completed
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    14 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--warning">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-placeholder">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    New order has been planced and pending for processing
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    20 mins
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--primary">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-share">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Database server overloaded 80% and requires quick reboot
                                                                    <span class="m-badge m-badge--info m-badge--wide">
                                                                        settled
                                                                    </span>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    1 hr
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--brand">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-exclamation-1">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    System error occured and hard drive has been shutdown -
                                                                    <a class="m-link" href="#">
                                                                        Check
                                                                    </a>
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    2 hrs
                                                                </span>
                                                            </div>
                                                            <div class="m-list-timeline__item">
                                                                <span class="m-list-timeline__badge m-list-timeline__badge--success">
                                                                </span>
                                                                <span class="m-list-timeline__icon flaticon-cogwheel-2">
                                                                </span>
                                                                <span class="m-list-timeline__text">
                                                                    Production server is rebooting...
                                                                </span>
                                                                <span class="m-list-timeline__time">
                                                                    3 hrs
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <span class="m-dropdown__arrow m-dropdown__arrow--left">
                                    </span>
                                </div>
                            </div>
                            <!--end::Dropdown-->
                        </div>
                    </div>
                    <!--end::Section-->
                </div>
            </div>
            <!--end::Portlet-->
        </div>
        <div class="col-lg-6">
            <!--begin::Portlet-->
            <div class="m-portlet">
                <div class="m-portlet__head">
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                                Search Result List
                            </h3>
                        </div>
                    </div>
                </div>
                <div class="m-portlet__body">
                    <!--begin::Section-->
                    <div class="m-section m-section--last">
                        <div class="m-section__content">
                            <!--begin::Preview-->
                            <div class="m-demo" data-code-html="true" data-code-js="false" data-code-preview="true">
                                <div class="m-demo__preview">
                                    <div class="m-list-search">
                                        <div class="m-list-search__results">
                                            <span class="m-list-search__result-message m--hide">
                                                No record found
                                            </span>
                                            <span class="m-list-search__result-category m-list-search__result-category--first">
                                                Documents
                                            </span>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-interface-3 m--font-warning">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Annual finance report
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-share m--font-success">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Company meeting schedule
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-paper-plane m--font-info">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Project quotations
                                                </span>
                                            </a>
                                            <span class="m-list-search__result-category">
                                                Customers
                                            </span>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-pic">
                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user1.jpg" title="">
                                                    </img>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Amanda Anderson
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-pic">
                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user2.jpg" title="">
                                                    </img>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Kennedy Lloyd
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-pic">
                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user3.jpg" title="">
                                                    </img>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Megan Weldon
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-pic">
                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user4.jpg" title="">
                                                    </img>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Marc-André ter Stegen
                                                </span>
                                            </a>
                                            <span class="m-list-search__result-category">
                                                Files
                                            </span>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-lifebuoy m--font-warning">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Revenue report
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-coins m--font-primary">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Anual finance report
                                                </span>
                                            </a>
                                            <a class="m-list-search__result-item" href="#">
                                                <span class="m-list-search__result-item-icon">
                                                    <i class="flaticon-calendar m--font-danger">
                                                    </i>
                                                </span>
                                                <span class="m-list-search__result-item-text">
                                                    Tax calculations
                                                </span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--end::Preview-->
                            <!--begin::Dropdown-->
                            <div class="m-dropdown m-dropdown--up m-dropdown--inline m-dropdown--huge m-dropdown--arrow" data-dropdown-toggle="click">
                                <a class="m-dropdown__toggle btn btn-warning dropdown-toggle" href="#">
                                    Dropdown example
                                </a>
                                <div class="m-dropdown__wrapper">
                                    <div class="m-dropdown__inner">
                                        <div class="m-dropdown__body">
                                            <div class="m-dropdown__content">
                                                <div class="m-scrollable" data-max-height="240" data-scrollable="true">
                                                    <div class="m-list-search">
                                                        <div class="m-list-search__results">
                                                            <span class="m-list-search__result-message m--hide">
                                                                No record found
                                                            </span>
                                                            <span class="m-list-search__result-category m-list-search__result-category--first">
                                                                Documents
                                                            </span>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-interface-3 m--font-warning">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Annual finance report
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-share m--font-success">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Company meeting schedule
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-paper-plane m--font-info">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Project quotations
                                                                </span>
                                                            </a>
                                                            <span class="m-list-search__result-category">
                                                                Customers
                                                            </span>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-pic">
                                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user1.jpg" title="">
                                                                    </img>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Amanda Anderson
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-pic">
                                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user2.jpg" title="">
                                                                    </img>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Kennedy Lloyd
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-pic">
                                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user3.jpg" title="">
                                                                    </img>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Megan Weldon
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-pic">
                                                                    <img class="m--img-rounded" src="../../assets/app/media/img/users/user4.jpg" title="">
                                                                    </img>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Marc-André ter Stegen
                                                                </span>
                                                            </a>
                                                            <span class="m-list-search__result-category">
                                                                Files
                                                            </span>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-lifebuoy m--font-warning">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Revenue report
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-coins m--font-primary">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Anual finance report
                                                                </span>
                                                            </a>
                                                            <a class="m-list-search__result-item" href="#">
                                                                <span class="m-list-search__result-item-icon">
                                                                    <i class="flaticon-calendar m--font-danger">
                                                                    </i>
                                                                </span>
                                                                <span class="m-list-search__result-item-text">
                                                                    Tax calculations
                                                                </span>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <span class="m-dropdown__arrow m-dropdown__arrow--left">
                                    </span>
                                </div>
                            </div>
                            <!--end::Dropdown-->
                        </div>
                    </div>
                    <!--end::Section-->
                </div>
            </div>
            <!--end::Portlet-->
        </div>
    </div>

@stop

@section('link-js')

@stop